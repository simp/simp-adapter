  ...
  context 'Initial test prep on each host' do
    ...
    specify do
      ...
      on(
        hosts,
        [
          %(runuser build_user -l -c "for i in {1..5}; do),
          %({ gpg2 --keyserver  hkps://keys.openpgp.org --recv-keys 7D2BAF1CF37B13E2069D6956105BD0E739499BDB),
          %(|| gpg2 --keyserver hkps://keyserver.ubuntu.com --recv-keys 7D2BAF1CF37B13E2069D6956105BD0E739499BDB;),
          %(} && break || sleep 1; done"),
        ].join(' '),
      )
      ...
      on(
        hosts,
        [
          'runuser build_user -l -c "curl -sSL https://raw.githubusercontent.com/rvm/rvm/stable/binscripts/rvm-installer -o rvm-installer',
          '&& curl -sSL https://raw.githubusercontent.com/rvm/rvm/stable/binscripts/rvm-installer.asc -o rvm-installer.asc',
          '&& gpg2 --verify rvm-installer.asc rvm-installer && bash rvm-installer"',
        ].join(' '),
      )
  ...
  hosts.each do |host|
    context "on host #{host}" do
      ...
      context 'Module RPM install' do
        ...
        it 'creates a tag for the installed module version' do
      ...
      context 'Module RPM reinstall when module repo exists' do
        it 'allows reinstalls' do
          on(host, module_reinstall_cmd)
        end

        it "does nothing to the module's central repo if current" do
      ...
      context 'Module RPM reinstall when module tag has been munged' do
        ...
        it 'replaces munged tag on reinstall' do
          on(host, module_reinstall_cmd)
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, mod_version, ['fixtures'])
        end
      end
      ...
      context 'Module RPM reinstall when module repo does not exist' do
        ...
        it "checks the reinstalled version into the repo's master branch" do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, 'master', ['fixtures'])
        end

        it 'creates a tag for the reinstalled module version' do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, mod_version, ['fixtures'])
        end
      end

      context 'Module RPM uninstall' do
        it 'uninstalls cleanly' do
        ...
        it 'does not remove the central repo for the module' do
          on(host, "test -d #{mod_repo_dir}")
        end

        it 'does not modify the central repo for the module' do
      ...
      context 'Skipped module RPM install' do
        it 'does not prevent module install' do
          host.install_package('pupmod-simp-site')
        end

        it 'does not create an accessible central repo for the module' do
      ...
      context 'Invalid module RPM install' do
        ...
        it 'does not prevent the module install' do
          # start clean
          on(host, "rm -rf #{mod_repo_dir}")
        ...
        it 'does not create the central repo for the module' do
          on(host, "test ! -d #{mod_repo_dir}")
        end
