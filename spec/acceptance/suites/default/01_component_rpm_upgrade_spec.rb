  ...
  hosts.each do |host|
    context "on host #{host.hostname}" do
      ...
      context 'Module RPM upgrade when module repo exists' do
        it 'installs the first version' do
          on(host, "yum install #{package1} -y")
        end

        it 'creates a central repo with the first version on the master branch' do
        ...
        it 'creates a tag for the first version' do
        ...
        it 'upgrades to the second version' do
          on(host, "yum upgrade #{package2} -y")
        end

        it "updates the repo's master branch with the second version" do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, 'master', ['fixtures'])
        end

        it 'creates a tag for the second version' do
        ...
        it 'stills have a tag for the first version' do
      ...
      context 'Module RPM downgrade' do
        it 'downgrades cleanly' do
          on(host, "yum downgrade #{package1} -y")
        end

        it "updates the repo's master branch with the older version" do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, 'master', ['fixtures'])
        end

        it 'has a tag for the older version' do
          on(host, "cd simp-beakertest-save1; git checkout tags/#{version1}")
          compare_to_repo_branch(host, '/root/simp-beakertest-save1', mod_repo_url, version1)
        end

        it 'still has a tag for the newer version' do
        ...
        it 'keeps history of version changes in git log' do
          on(host, 'rm -rf /root/simp-beakertest*')
      ...
      context 'Module RPM upgrade when module repo does not exist' do
        it 'upgrades cleanly' do
          on(host, "rm -rf #{mod_repo_dir}")
          on(host, "yum upgrade #{package2} -y")
        end

        it 'creates a central repo with the newer version on the master branch' do
          on(host, "test -d #{mod_repo_dir}")
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, 'master', ['fixtures'])
        end

        it 'creates a tag for the newer version' do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, version2, ['fixtures'])
        end
      end
      ...
      context 'Module RPM downgrade when module repo does not exist' do
        ...
        it 'creates a central repo with the older version on the master branch' do
          on(host, "test -d #{mod_repo_dir}")
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, version1, ['fixtures'])
        end

        it 'creates a tag for the older version' do
          compare_to_repo_branch(host, mod_install_dir, mod_repo_url, version1, ['fixtures'])
        end
      end
    end
