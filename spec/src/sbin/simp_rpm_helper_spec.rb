...
Usage: #{script} -d DIR -s SECTION -S STATUS [options]
  ...
  describe 'run' do
    ...
    context 'options error cases' do
      it 'fails and print help with invalid option' do
        expected = <<-EOF
...
#{usage.strip}
      ...
      it 'fails and print help if rpm_dir option is missing' do
        expected = <<-EOF
...
#{usage.strip}
      ...
      it 'fails if rpm_dir option is not an absolute path' do
      ...
      it 'fails and print help if rpm_status option is missing' do
        expected = <<-EOF
...
#{usage.strip}
      ...
      it 'fails and print help if rpm_section option is missing' do
        expected = <<-EOF
...
#{usage.strip}
      ...
      it 'fails and print help if invalid rpm_section option is specified' do
        expected = <<-EOF
...
#{usage.strip}
      ...
      ['posttrans', 'preun' ].each do |rpm_section|
        it "fails if rpm_dir is not a found for #{rpm_section}" do
      ...
      it 'fails if rpm_status is not an integer' do
      ...
      it 'fails if work_dir option is not an absolute path' do
    ...
    context 'config file error cases' do
      it 'fails if specified config file does not exist' do
      ...
      it 'fails if specified config file cannot be parsed' do
        expected = %r{#{Regexp.escape("#{script} ERROR: Config file '#{__FILE__}' could not be processed")}}
      ...
      it 'fails if target_dir from config file is not an absolute path' do
      ...
      it 'fails if work_dir from config file is not an absolute path' do
    ...
    context 'config file defaults exist' do
      ...
      it 'uses defaults for unspecified options' do
        ...
        one_verbose_msg = %r{Repo update completed} # spot check one of the verbose messages
      ...
      it 'uses command line options in lieu of defaults' do
        config = {
    ...
    context 'missing required commands' do
      ...
      it 'fails when rsync is command missing' do
        ...
        expect { SimpRpmHelper.new }.to raise_error(RuntimeError,
          %r{cannot locate 'rsync' executable})
      end
    end
    ...
    context 'other failures' do
      ...
      let(:config_file) { File.join(tmp_dir, 'adapter_conf.yaml') }
      let(:module_repo_dir) { File.join(config['target_dir'], 'simp-beakertest.git') }
      ...
      it 'fails when git init fails' do
  ...
  describe 'execute' do
    ...
    it 'returns log hash upon success' do
    ...
    it 'fails when command fails' do
      ...
      SimpRpmHelper::CommandError, %r{ls: cannot access}
    )
    end

    it 'fails with specified message title when command fails' do
      expect { helper.execute('ls /does/not/exist', 'Failed to find required dir') }
        .to raise_error(SimpRpmHelper::CommandError, %r{Failed to find required dir})
    end
  end
end
